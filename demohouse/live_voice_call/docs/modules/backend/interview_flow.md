...
## 模块职责
- 负责 `interview_flow` 相关逻辑（基于文件命名与代码结构）。
...
## 对外接口（函数/类）
L18: class QuestionContext:
L29: class FlowResponse:
L38: class InterviewFlow:

## 依赖与配置
- 关键导入（节选）：
L1: from dataclasses import dataclass, field
L2: from typing import Any, Dict, List, Optional
L4: from interview_judge import Decision, InterviewJudge
