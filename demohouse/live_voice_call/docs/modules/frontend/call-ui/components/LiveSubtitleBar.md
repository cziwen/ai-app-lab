# frontend/src/call-ui/components/LiveSubtitleBar.tsx

## 模块职责
- 负责 `call-ui/components/LiveSubtitleBar` 对应的前端能力（页面/组件/hook/类型/工具）。
...
## 对外接口（导出项）
L3: export const LiveSubtitleBar = ({ text }: LiveSubtitleBarProps) => {

## 依赖与配置
- 关键导入（节选）：
L1: import type { LiveSubtitleBarProps } from '@/call-ui/types';
