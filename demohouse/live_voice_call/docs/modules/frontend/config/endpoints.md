...
## 模块职责
- 负责 `config/endpoints` 对应的前端能力（页面/组件/hook/类型/工具）。
...
## 对外接口（导出项）
L24: export const WS_URL = readEnv('MODERN_PUBLIC_WS_URL') ?? fallbackWsUrl();
L25: export const LOG_URL = readEnv('MODERN_PUBLIC_LOG_URL') ?? `${fallbackOrigin()}/api/frontend-logs`;
L26: export const API_URL = readEnv('MODERN_PUBLIC_API_URL') ?? fallbackOrigin();
