# frontend/src/utils/log_sender.ts

## 模块职责
- 负责 `utils/log_sender` 对应的前端能力（页面/组件/hook/类型/工具）。
...
## 对外接口（导出项）
L133: export const logSender = new LogSender();

## 依赖与配置
- 关键导入（节选）：
L12: import { LOG_URL } from '@/config/endpoints';
