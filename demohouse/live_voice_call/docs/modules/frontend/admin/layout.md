...
## 模块职责
- 负责 `admin/layout` 对应的前端能力（页面/组件/hook/类型/工具）。
...
## 对外接口（导出项）
L15: export const AdminShell = ({
L65: export const AdminLoadingPage = () => (
L77: export const AdminModal = ({ title, onClose, children }: AdminModalProps) => (

## 依赖与配置
- 关键导入（节选）：
L1: import { ReactNode } from 'react';
L2: import { useNavigate } from '@modern-js/runtime/router';
