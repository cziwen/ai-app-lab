# frontend/src/admin/use-admin-auth.ts

## 模块职责
- 负责 `admin/use-admin-auth` 对应的前端能力（页面/组件/hook/类型/工具）。
...
## 对外接口（导出项）
L5: export const useAdminAuth = () => {

## 依赖与配置
- 关键导入（节选）：
L1: import { useEffect, useState } from 'react';
L2: import { useNavigate } from '@modern-js/runtime/router';
L3: import { adminApi } from '@/admin/api';
