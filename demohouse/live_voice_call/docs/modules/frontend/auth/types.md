...
## 模块职责
- 负责 `auth/types` 对应的前端能力（页面/组件/hook/类型/工具）。
...
## 对外接口（导出项）
L1: export type PermissionStatus = 'pending' | 'granted' | 'denied';
L3: export type PermissionKey = 'mic' | 'camera' | 'screen' | 'speaker';
L5: export interface PermissionState {
