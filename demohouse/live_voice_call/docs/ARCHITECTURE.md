...
- `问题` 每行必填。
- `场景` 可空，空值表示延续当前场景连续段（子问）。
- `评分标准` 与 `最大分数` 仅场景首问填写，子问必须为空。

### LLM2 上下文窗口（按场景连续段隔离）

`VoiceBotService` 在会话启动时按“连续场景段”构建上下文段 ID：
- 相邻题目场景相同：归入同一段，共享追问/澄清/子问历史。
- 场景切换：开启新段并切换上下文窗口。
- 同名场景跨段再次出现：视为新段，不回连旧段历史。
- 空场景题目：按单题段处理（不与其他段合并）。

每轮 LLM2 输入为：`当前段历史 + 当前轮指令`，而非全局历史。

### LLM3 评分（按场景连续段整题评分）

评分阶段先把问答快照按“场景连续段”聚合，再逐段调用 LLM3：
- 一段只调用一次评分模型。
- 评分输入包含：`场景名 + 子问拼接 + 合并回答 + 段首评分标准 + 段首最大分数`。
- 输出契约固定为：`numeric_score` 与 `comment`（评语含得分原因/过程）。

```mermaid
flowchart LR
  A["CSV v2 题库"] --> B["按连续场景切分 segment"]
  B --> C["面试阶段: 子问逐条提问"]
  C --> D["LLM2 仅加载当前 segment 历史"]
  C --> E["产生逐轮问答快照"]
  E --> F["评分阶段: 按 segment 聚合"]
  F --> G["LLM3 每个 segment 调用一次"]
  G --> H["输出 numeric_score + comment"]
```
